...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
            <h3 class="text-lg font-bold text-gray-800">بيانات العرض الجديد</h3>
        ...
        <form action="{{ route('admin.offers.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name_ar" value="{{ old('name_ar') }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" required>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">اسم العرض (إنجليزي) <span class="text-red-500">*</span></label>
                    <input type="text" name="name_en" value="{{ old('name_en') }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" required>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">الوصف (عربي)</label>
                    <textarea name="description_ar" rows="3" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all resize-none">{{ old('description_ar') }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">الوصف (إنجليزي)</label>
                    <textarea name="description_en" rows="3" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all resize-none">{{ old('description_en') }}</textarea>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
                    ...
                    <label class="cursor-pointer">
                        <input type="radio" name="type" value="{{ $type }}" {{ old('type') === $type ? 'checked' : '' }} class="hidden peer" onchange="showTypeFields(this.value)" required>
            ...
            <div id="fields-percentage" class="type-fields hidden">
                <div class="p-5 bg-purple-50 rounded-xl border border-purple-100">
                    ...
                    <input type="number" name="value" value="{{ old('value') }}" min="1" max="100" step="0.01" class="w-full md:w-48 px-4 py-3 border border-purple-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-400" placeholder="20">
                </div>
            </div>
            ...
            <div id="fields-fixed" class="type-fields hidden">
                <div class="p-5 bg-blue-50 rounded-xl border border-blue-100">
                    ...
                    <input type="number" name="value" value="{{ old('value') }}" min="0" step="0.01" class="w-full md:w-48 px-4 py-3 border border-blue-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="50">
                </div>
            </div>
            ...
            <div id="fields-bundle" class="type-fields hidden">
                <div class="p-5 bg-orange-50 rounded-xl border border-orange-100">
                    ...
                    <input type="number" name="bundle_price" value="{{ old('bundle_price') }}" min="0" step="0.01" class="w-full md:w-48 px-4 py-3 border border-orange-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="300">
                    <p class="text-xs text-orange-600 mt-2">سيتم تطبيق هذا السعر عند شراء جميع منتجات الباقة معاً.</p>
                </div>
            </div>
            ...
            <div id="fields-buy_x_get_y" class="type-fields hidden">
                <div class="p-5 bg-green-50 rounded-xl border border-green-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">اشترِ (الكمية المطلوبة)</label>
                            <input type="number" name="buy_quantity" value="{{ old('buy_quantity') }}" min="1" class="w-full px-4 py-3 border border-green-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-400" placeholder="2">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">احصل على (الكمية المجانية)</label>
                            <input type="number" name="get_quantity" value="{{ old('get_quantity') }}" min="1" class="w-full px-4 py-3 border border-green-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-400" placeholder="1">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">المنتج المجاني (اتركه فارغاً لنفس المنتج)</label>
                            <select name="get_product_id" class="w-full px-4 py-3 border border-green-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-400 bg-white">
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" {{ old('get_product_id') == $product->id ? 'selected' : '' }}>{{ $product->name_ar }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="p-5 bg-pink-50 rounded-xl border border-pink-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="min_spend" value="{{ old('min_spend') }}" min="0" step="0.01" class="w-full px-4 py-3 border border-pink-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-400" placeholder="500">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">قيمة الخصم (ج.م)</label>
                            <input type="number" name="discount_amount" value="{{ old('discount_amount') }}" min="0" step="0.01" class="w-full px-4 py-3 border border-pink-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-400" placeholder="100">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mb-3">اختر المنتجات التي يسري عليها هذا العرض. إذا لم تختر أي منتج، سيسري العرض على جميع المنتجات (للأنواع: نسبة مئوية، مبلغ ثابت، اشترِ X).</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="datetime-local" name="starts_at" value="{{ old('starts_at') }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">تاريخ الانتهاء</label>
                    <input type="datetime-local" name="expires_at" value="{{ old('expires_at') }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="flex items-end">
                    <label class="flex items-center gap-3 cursor-pointer pb-3">
                        <input type="hidden" name="is_active" value="0">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', '1') == '1' ? 'checked' : '' }} class="w-5 h-5 rounded text-blue-600 border-gray-300 focus:ring-blue-500">
                        <span class="text-sm font-bold text-gray-700">تفعيل العرض فوراً</span>
                    </label>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" class="px-8 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-all shadow-sm">حفظ العرض</button>
            </div>
        </form>
    </div>
</div>
...
// Show on load if returning from validation error
